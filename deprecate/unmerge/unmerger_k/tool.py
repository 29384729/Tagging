...
def build_parent_targets_from_group_size(
    ...
    *,
    ...
    if kind in ("missing", "delta", "deltan"):
        kk = np.maximum(gs - 1, 0)  # Track constituent merge groups and parent/child indices.
    else:
        kk = np.maximum(gs, 0)  # Track constituent merge groups and parent/child indices.
    ...
    *,
    ...
    for ep in range(1, int(cfg.epochs) + 1):
        ...
        for batch in train_loader:
            ...
            out = model(x, m, parent_idx=None)
            parent_loss = _masked_bce_with_logits(out.parent_logit, y_parent, m, pos_weight=float(pos_weight))  # Collect GT parent samples.
            w = torch.where(y_parent > 0.5, torch.ones_like(y_parent), y_parent.new_full(y_parent.shape, float(cfg.neg_k_weight)))
            if out.k_logits is not None:
                K = int(out.k_logits.shape[-1])
                y_ki = torch.clamp(y_k.round().to(torch.long), 0, K - 1)
                ...
                k_loss = _masked_weighted_huber(out.k_pred, y_k, m, w, delta=float(cfg.huber_delta))  # Collect GT parent samples.
            sum_loss = _parent_sum_huber(out.k_pred, y_k, y_parent, m, delta=float(cfg.huber_delta))
    ...
    for s0 in range(0, N, bs):
        ...
        for bi, j in enumerate(range(s0, s1)):
            ...
            if parents.size > 0:
                ...
                for pi in range(P):
                                                            # Compute k targets, losses, and inference choices.
