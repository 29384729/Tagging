...
- Heads:
  - parentness: per-token logit ( token  merged parent)
  - reconstruction: given a parent token index (teacher forcing in training),
    generate Kmax child features in a fixed ordered layout ( offline pt ).
  - objectness: per-slot logit ( slot  child)
...
class OrderedUnmerger(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        # dR-based attention is optional and active only in the encoder.
        coords = None
...
def obj_prob_from_logits(obj_logit: torch.Tensor) -> torch.Tensor:
    """Convert object logits to probabilities."""
