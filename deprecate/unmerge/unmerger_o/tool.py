...
：
-  k；decoder  Kmax  slot  child_feat， slot  objectness。
-  prefix+threshold（） parent  children。
...
def apply_hlt_effects_collect_samples(
    ...
    *,
    ...
    samples: List[Tuple[int, int]] = []  # Collect GT parent samples.
    children: List[np.ndarray] = []  # Track constituent merge groups and parent/child indices.
    ...
    if bool(hcfg["merge_enabled"]) and float(hcfg["merge_radius"]) > 0:
        ...
        for jet_idx in range(n_jets):
            valid_idx = np.where(hlt_mask[jet_idx])[0]  # Track constituent merge groups and parent/child indices.
            if len(valid_idx) == 0:
                continue
            ...
            if len(valid_idx) >= 2:
                to_remove = set()  # Track constituent merge groups and parent/child indices.
                for a in range(len(valid_idx)):
                    ...
                    for b in range(a + 1, len(valid_idx)):
                        ...
                        if dR < r:
                            ...
                            if pt_sum < 1e-6:
                                continue
                            g[i].extend(g[j])  # Track constituent merge groups and parent/child indices.
                            g.pop(j, None)
                            group_size[jet_idx, i] = int(group_size[jet_idx, i] + group_size[jet_idx, j])
                            group_size[jet_idx, j] = 0  # Set an integer to fix a specific jet.
                            ...
                            to_remove.add(j)  # Track constituent merge groups and parent/child indices.
            ...
            for p, ch in g.items():
                ...
                if int(ch_arr.shape[0]) > int(k_max):
                    ch_arr = ch_arr[: int(k_max)]  # Track constituent merge groups and parent/child indices.
                samples.append((int(jet_idx), int(p)))  # Collect GT parent samples.
    ...
    if len(samples) > 0:
        ...
        for (j, p), ch in zip(samples, children):
            if bool(hlt_mask[int(j), int(p)]):
                s2.append((int(j), int(p)))  # Collect GT parent samples.
                c2.append(ch)  # Track constituent merge groups and parent/child indices.
...
def build_parent_targets_from_group_size(hlt_mask: np.ndarray, group_size: np.ndarray) -> np.ndarray:
    ...
    parent_gt = (hlt_mask & (gs > 1)).astype(np.float32)  # Collect GT parent samples.
    return parent_gt
...
class ParentRecoDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        out = {
            ...
            "tgt": tgt,
            "tgt_mask": tgt_mask,  # Objectness and slot-level diagnostics.
            "m_true": torch.tensor(float(m), dtype=torch.float32),
        }
        # px/py conservation auxiliary loss uses the HLT raw parent token and jet_phi.
...
def _slot_bce_with_logits(
    ...
    *,
    ...
    Args:
      logits: [B,K]
      targets_bool: [B,K] bool (True= child)
    """
...
def prefix_lengths_from_prob(prob: torch.Tensor, tau: float) -> torch.Tensor:
    """Objectness and slot-level diagnostics."""
    if prob.ndim != 2:
...
class TrainCfgRecoObj:
    ...
    thr_obj: float = 0.5
    # Post-Stage2 visualization and diagnostics.
...
def _pxy_loss_B(
    ...
    *,
    ...
    """
    px/py （B，）， GT slot(tgt_mask) 。
    Args:
      pred_child_std: [B,K,D]  child 
      tgt_mask: [B,K] bool
      feat_means/stds: [D]
...
def eval_parentness(model, loader, device: torch.device, *, pos_weight: float, cfg: TrainCfgParent) -> dict[str, float]:
    ...
    prec = tp / max(1, (tp + fp))
    rec = tp / max(1, (tp + fn))
    # Summarize repeat ROC / AUC / FPR@TPR / Gap Recovery and report mean and variance.
    try:
...
def eval_reco_obj(
    ...
    *,
    ...
    for batch in loader:
        ...
        # Parentness comparison before and after reconstruction training.
