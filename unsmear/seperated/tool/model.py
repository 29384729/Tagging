...
class _ConvBlock1D(nn.Module):
    """Small Conv1d block used by TokenUNet1D."""

    def __init__(self, in_ch: int, out_ch: int, *, dropout: float):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv1d(int(in_ch), int(out_ch), kernel_size=3, padding=1),
            nn.GroupNorm(1, int(out_ch)),
            nn.GELU(),
            nn.Dropout(float(dropout)),
            nn.Conv1d(int(out_ch), int(out_ch), kernel_size=3, padding=1),
            nn.GroupNorm(1, int(out_ch)),
            nn.GELU(),
        )
...
class TokenUNet1D(nn.Module):
    """Sequence U-Net regressor for token-level unsmearing."""
    ...
    def __init__(
        ...
        *,
        ...
        in_ch = self.input_dim + (1 if self.add_mask_channel else 0)
        channels = [self.base_channels * (2 ** i) for i in range(self.depth)]
        self.down_blocks = nn.ModuleList()
        self.pools = nn.ModuleList()
        prev_ch = in_ch
        for ch in channels:
            self.down_blocks.append(_ConvBlock1D(prev_ch, ch, dropout=float(dropout)))
            self.pools.append(nn.AvgPool1d(kernel_size=2, stride=2, ceil_mode=True))
            prev_ch = ch

        self.bottleneck = _ConvBlock1D(prev_ch, prev_ch * 2, dropout=float(dropout))
        bottleneck_ch = prev_ch * 2
        self.up_blocks = nn.ModuleList()
        self.up_projs = nn.ModuleList()
        for ch in reversed(channels):
            self.up_projs.append(nn.Conv1d(bottleneck_ch, ch, kernel_size=1))
            self.up_blocks.append(_ConvBlock1D(ch * 2, ch, dropout=float(dropout)))
            bottleneck_ch = ch

        out_ch = (2 * self.input_dim) if self.predict_logvar else self.input_dim
        self.out = nn.Conv1d(channels[0], out_ch, kernel_size=1)
        self._init_weights()
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor):
        ...
        if self.add_mask_channel:
            inp = torch.cat([inp, mask.to(dtype=x.dtype).unsqueeze(-1)], dim=-1)
        h = inp.transpose(1, 2)
        mask_1d = mask.to(dtype=h.dtype).unsqueeze(1)
        ...
        for block, pool in zip(self.down_blocks, self.pools):
            ...
            skips.append(h)
            h = pool(h)
            mask_1d = pool(mask_1d).clamp_max(1.0)

        h = self.bottleneck(h)
        for proj, block, skip in zip(self.up_projs, self.up_blocks, reversed(skips)):
            h = torch.nn.functional.interpolate(h, size=skip.shape[-1], mode="linear", align_corners=False)
            h = proj(h)
        ...
        if out.shape[-1] != seq_len:
            out = torch.nn.functional.interpolate(out, size=seq_len, mode="linear", align_corners=False)
...
class _TimeEmbed(nn.Module):
    ...
    The old implementation using sin(2*pi*t), cos(2*pi*t) makes t=0 and t=1 identical (endpoints are indistinguishable).
