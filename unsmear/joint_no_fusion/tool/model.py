...
Both models reuse `SharedTokenEncoder` and `SharedJetClassifierHead` so the
baseline classifier trunk and the joint classifier trunk stay structurally
aligned.
"""
...
class SharedJetClassifierHead(nn.Module):
    ...
    def __init__(self, *, embed_dim: int, dropout: float, pool_num_heads: int = 4):
        super().__init__()
        self.embed_dim = int(embed_dim)
        self.pool_num_heads = int(pool_num_heads)
        if self.embed_dim % self.pool_num_heads != 0:
            raise ValueError(
                f"embed_dim={self.embed_dim} must be divisible by pool_num_heads={self.pool_num_heads}"
            )
        self.pool_query = nn.Parameter(torch.randn(1, 1, self.embed_dim) * 0.02)
        self.pool_attn = nn.MultiheadAttention(
            self.embed_dim, num_heads=self.pool_num_heads, dropout=float(dropout), batch_first=True
        )
...
class ParticleTransformerKD(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        max_seq_len: int = 128,
        mask_encoder_input: bool = False,
        cls_pool_heads: int = 4,
    ):
        super().__init__()
        self.input_dim = int(input_dim)
        self.embed_dim = int(embed_dim)
        self.cls_pool_heads = int(cls_pool_heads)
        ...
        self.cls_head = SharedJetClassifierHead(
            embed_dim=int(embed_dim),
            dropout=float(dropout),
            pool_num_heads=int(cls_pool_heads),
        )
